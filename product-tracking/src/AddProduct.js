  ...
  const handleUploadProduct = async (e) => {
    ...
    if (glbFile) {
      try {
        ...
        await axios.post('http://127.0.0.1:5000/uploadModel', {
          ID: id,
    ...
    Papa.parse(csvBatchFile, {
      complete: async (result) => {
        for (let row of result.data) {
          ...
          try {
            const token = localStorage.getItem('token');
            await axios.post('http://127.0.0.1:5000/uploadBatch', postBatchData, {
              headers: {
                'Authorization': `Bearer ${token}`
              }
            });
            ...
            <p style={{ color: "grey" }}>
              ...
              {messageProduct && !viewProduct && (
                <div>
                  {lastAddedProduct && (
                    <div>
                      <QRCodeCanvas value={lastAddedProduct} style={{ marginBottom: "2vw" }} />
                      <p>Product ID: <b>{lastAddedProduct}</b></p>
                    </div>
                  )}
                </div>
              )}
              {!viewProduct &&
                <button
                  ...
                  onClick={() => { setViewProduct(true); setViewBatch(false); setMessageProduct('') }}
                >
                  + New Product
                </button>
              }
                ...
                <Card.Body>
                  ...
                  <Form.Group className="d-flex align-items-center mb-3" style={{ marginTop: "1vw" }}>
                    ...
                    >
                      ...
                      { csvFile ? JSON.stringify(csvFile.name) : "upload data Batch from CSV file" } 
                      </label>
                      <input
                        style={{ display: "none" }}
                        id="csvFile"
            ...
            <p style={{ color: "grey" }}>
              ...
              {messageBatch && !viewBatch && (
                <div>
                  {lastAddedBatch && (
                    <div>
                      <QRCodeCanvas value={lastAddedBatch} style={{ marginBottom: "2vw" }} />
                      <p>Batch ID: <b>{lastAddedBatch}</b></p>
                    </div>
                  )}
                </div>
              )}
              {!viewBatch &&
                <button
                  className="btn btn-primary mt-3 w-100"
                  onClick={() => { setViewBatch(true); setViewProduct(false); setMessageBatch('') }}
                >
                  + New Batch
                </button>
              }
              {viewBatch &&
                <Card.Body>
                  {/* CSV File Upload */}
                  <Form.Group className="d-flex align-items-center mb-3" style={{ marginTop: "1vw" }}>
                    <Form.Label style={{ width: inputWidth }} className="me-3">Upload CSV Batch</Form.Label>
                    <Form
                      style={{
                        display: "flex",
                        width: "100%",
                    ...
                    >
                      {csvBatchFile ?
                        <ion-icon
                          name="trash-outline"
                          style={{ cursor: "pointer", color: "grey" }}
                          onClick={() => { document.getElementById("csvBatchFile").value = ''; setCsvBatchFile('') }}
                        />
                        :
                        <ion-icon name="folder-outline" style={{ color: "grey" }}></ion-icon>
                      }
                      <label htmlFor='csvBatchFile' style={{ color: "grey", textDecoration: "underline", cursor: "pointer" }}>
                  ...
                  {customBatchFields.map((field, index) => (
                    ...
                    <Button variant="primary" onClick={addCustomBatchField} className="my-3 d-block mx-auto">+ Add Batch Field</Button>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', margin: '20px 0' }}>
                    ...
                    <hr style={{ flex: 1, border: 'none', borderTop: '1px solid #666' }} />
                      ...
                      <Button variant="primary" type="submit" disabled={!idBatch || !productId || !batchNumber || !quantity || !productionDate}>
                      Upload Batch
                    </Button>
                  </Form>
                </Card.Body>
